...
int main(int argc, char *argv[]) {
    ...
    printf("Digite um ID(username) para se identificar: ");
    fgets(id, USER_SIZE, stdin);


    while (1) {
        // Solicita ao usuário uma mensagem
        printf("Digite uma mensagem para enviar ao servidor (ou digite '@sair' para encerrar): ");
        fgets(message, BUFFER_SIZE, stdin);

        // Verifica se a mensagem é '@sair' para encerrar o cliente
